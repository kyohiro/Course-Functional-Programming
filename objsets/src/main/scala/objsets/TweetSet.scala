...
abstract class TweetSet {
  ...
  /**
   *  Add one additional function to determine whether the set is empty or not
   */ 
  def isEmpty:Boolean
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val accum = left.filterAcc(p, right.filterAcc(p, acc))
    if (p(elem)) accum.incl(elem)
    else accum 
  }
  
  def union(that: TweetSet): TweetSet = left union (right union (that incl elem))

  //Added one additional method isEmpty to the set interface
  //Not aware of the better approach to avoid this
  def mostRetweeted: Tweet = {
  ...
  def descendingByRetweet: TweetList = {
    val mRetweet = mostRetweeted
    new Cons(mRetweet, remove(mRetweet).descendingByRetweet)
  }
